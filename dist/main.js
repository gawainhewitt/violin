...
let one = 'violinLoop1';
let two = 'violinLoop2';
let three = 'violinLoop3';
let four = 'violinLoop4';
let five = 'violinLoop5';
let six = 'violinLoop6';

let stepName = new Array;

for(let i = 0; i < seqRows; i++){
  stepName[i] = `violinStep${i}`;
}
...
function welcomeScreen() {
  ...
  text("violin", width/2, height/10 * 2);
  textSize(cnvDimension/20);
  text( "Containing excerpts from Bartok's Cushion Dance performed by Ann Lovatt.", width/10, height/10, (width/10) * 8, (height/10) * 8);
...
function startAudio() {
  ...
  Tone.Transport.scheduleRepeat(play_ = () => {player1.start();}, '2n');
  ...
  Tone.Transport.scheduleRepeat(play_ = () => {player5.start();}, '2m');
  Tone.Transport.scheduleRepeat(play_ = () => {player6.start();}, '2n');
  retrieveSavedWork();
}
